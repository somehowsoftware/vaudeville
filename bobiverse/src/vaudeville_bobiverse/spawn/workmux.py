...
def workmux_add_argv(worktree: str, foundation_session: str, agent: str) -> list[str]:
    # No --prompt-file: the launch turn reaches claude on the agent script's stdin, so workmux
    # carries only the fork and the agent script, never a byte of the turn's content in argv.
    return [
        "workmux",
        "add",
        worktree,
...
def workmux_invocation(
    ...
    argv = workmux_add_argv(worktree, seeded.foundation_session, str(agent_script))
