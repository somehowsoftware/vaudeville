...
@app.command(
    ...
    help=(
        ...
        "no Foundation fork. The Brief reaches claude on stdin, never argv, so no "
        "`pkill -f` matching a word inside it can reap the agent. The intra-session "
        "sibling of spawn and teardown; cue's `vv checkpoint` launches it as the "
        "detached handoff that sheds an oversized conversation. Refuses before the "
        "destructive respawn -- leaving the live session untouched -- when the pane is gone."
    ),
)
def reseat_command(
...
@app.command(
    ...
    help=(
        ...
        "and print its path. /spawn hands that path to the agent script, which "
        "redirects the launcher onto the new Bob's stdin as its first turn."
    ),
)
