...
- **The spec** carries the high-level WHAT. One paragraph to one page per bounded context, at a level that survives many code changes. A spec that churns with every refactor has slipped into territory tests should pin.
...
The ubiquitous language sits alongside as a reference vocabulary — terms defined in relation to one another, distinct from the spec. Cross-context UL lives in [vocabulary](../vocabulary.md); context-local UL lives in each bounded context's own UL doc, which references the cross-context vocabulary rather than redefining it.

## The bounded context is the repo

No document marks where one bounded context ends and another begins: the repository's filesystem boundary is the marker. A `bounded-context.md` (or similar) is a smell — such a file mixes spec, vocabulary, and behaviour description, and rots as the code drifts.
...
Priming-time and skill-text exhortations are deliberately not the load-bearing mechanism — too distant from the action point to pull the outcome reliably.
