...
A *piece* is a domain noun — something the [vocabulary](../vocabulary.md) already names or is about to name. A subprocess invocation is not a piece — it is implementation; the piece is the domain concept the subprocess implements.
...
If the piece-and-interaction pass finds a piece with no name in the ubiquitous language, the finding is the missing name. Add the term to the cross-context [vocabulary](../vocabulary.md) (or the relevant bounded context's UL doc), then continue.
