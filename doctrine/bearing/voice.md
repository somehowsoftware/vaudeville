...
How an agent writes when its output is read by the operator — chat turns, commit messages, Premise descriptions, plans, implementation writeups, anything readable. [language](../code/language.md) governs the words that name the *system*; this document governs how the agent expresses itself in the output that carries that naming. It applies alongside the methodology in [practice](../practice/), on every output channel.
...
This rule governs every output channel — commit messages, Premise descriptions, implementation writeups, ordinary replies — not only check-ins.

## No temporal anchors

Do not write durations ("a few hours", "a week of work"), framing-level duration cues ("a small spike", "a quick first cut"), or calendar / relative-time anchors ("around when X closes", "soon", "later this week") in chat turns, Premise descriptions, plans, commit messages, or any other output. State the dependency relation instead: "after Premise X lands", "gated on Y being merged", "blocked on Z", "next in dependency order after W". Scope words (number of Premises, acceptance criteria, in-scope vs. out-of-scope) are fine; time words are not.
