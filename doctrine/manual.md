...
**The installed tool ships no command to build this schema for you** — provisioning a project is on neither `vv` nor `vaudeville`. Standing one up today is a manual YouTrack operation: create the project, add the fields and the link type, wire the state/workflow bundles, and set the essential defaults, all by hand. On a YouTrack instance that already runs a Vaudeville project, replicate that project's fields, bundles, and defaults onto the new one; the first project on a fresh instance has no exemplar and must be configured field by field.

The authoritative field list, states, and link types are not yet written into this universal layer — they live in the framework's own build machinery, which a tenant does not receive. That gap is tracked as **CORE-75**; until it closes, treat provisioning as hand-work and prove it by filing and moving one real Assignment end to end.
...
Your entire shell surface is one command, `vaudeville`, with four verbs — `bob`, `pickable`, `prime`, `spawn`. Everything else you do happens inside an agent conversation, through skills you invoke with `/`. You never type `vv`: that is the agents' own 34-verb facade, [Component](vocabulary.md#component)-scoped, driven by the skills and the machinery on your behalf. Run the loop end to end once:
...
That table is the whole of your shell: four `vaudeville` verbs, and the rest done by talking to agents. If you find yourself reaching for `vv` at a prompt, you have stepped onto the agents' surface, not yours.
...
- **Tracker provisioning is hand-work.** The installed tool ships no command to build a project's schema; standing up a Component's tracker is a manual YouTrack setup, and the authoritative field/state/link list is not yet in the universal layer. Tracked as CORE-75.
