...
**CLI Declaration.** A Contributor Repo's declaration that it contributes to the federated command line: the `[tool.vaudeville]` table, naming a `binary` whose entry point identifies its `vv` app, an `operator_binary` whose entry point identifies its operator app for [`vaudeville`](#vaudeville), or both. Its presence marks the Contributor as part of the command [Surface](#surface), so it names at least one of the two: a table naming neither claims a Surface it does not have. Contributors with no CLI omit the table; a Contributor with a `vv` app but no operator app omits `operator_binary`, and one offering only an operator app omits `binary`. The command surface itself is whatever the apps define; Ringmaster reads it from them, never from a hand-maintained list. (Named CLI Declaration, not Manifest, so the Contributor-side `[tool.vaudeville]` table does not collide with the integrated [Manifest](#manifest) Survey produces.)
...
**Release Notes.** The human-readable synthesis of a [Changeset](#changeset): what changed since the [Predecessor](#predecessor), read across the Contributors. Named here as the work it describes; the synthesis is authored by judgment, not a mechanized operation — only its input, the Changeset, is built.
...
**Builder.** The integrator identity and version that built a [Release](#release): the [Facade](#facade) it stamped and the installer it built, recorded by its running version (`vaudeville_ringmaster` and `vaudeville_install`). Recorded in the [Provenance](#provenance), never itself a [Pin](#pin): the integrator is what builds the Release, not a payload in its [Carried Set](#carried-set). It appears in the [Exposition](#exposition) as its own directory, exactly where it is absent from the Provenance's carried record.

**Exposition.** A [Release](#release)'s for-reading rendering of its assembled source: one directory per Contributor, each named for the Contributor and holding that Contributor's readable material — its `src`, any workspace `packages` it ships (such as vaudeville-ringmaster's carried installer), its `docs`, its `README`, and the skills it defines, each lifted up to where a reader browsing the directory lands rather than left in the source-side [Contribution Layout](#contribution-layout) (`scaffold/.claude/…`) that files it for the build. A reading copy, deliberately *not* the build input: it lifts each Contributor's readable material into its own directory, trimmed of tests, tooling, lockfiles, history, and the source-side placement machinery, and is regenerated whole each Publish. The universal Doctrine is one Contributor among the rest, rendered the same way into its own `doctrine/` directory rather than as a special case. What the Release was actually built from is its [Provenance](#provenance), placed at the Exposition's root beside the Contributor directories, not the rendering. [Publish](#publish) commits the Exposition in the [Published Home](#published-home) and tags the Release there, so a reader browses the whole constellation at the Release's tag without cloning each Contributor Repo. But that committing is an act Publish performs on the Exposition, not part of what the Exposition is. Distinct from the [Artifact](#artifact), which carries the same code as built wheels rather than readable source.
...
**Config Origin.** Which config dir a host's [Tenant Config](#tenant-config) was placed from, recorded on the host as installer bookkeeping when it is placed and read back by every later operator command. Where the Tenant Config is *what* an installation serves, the Config Origin is *where that answer came from*, and it exists because only the host can hold it: the config dir names a real production location, so a command that resolves one for itself resolves it for a tenant whose config lives somewhere it cannot know. [Refresh](#refresh) and [Update](#update) take a declared `--config-dir` when the operator supplies one — which, being placed, records — and the recorded origin otherwise; a host with neither refuses rather than guessing. Distinct from the [Registry](#registry), which is integrator-internal and never sourced from a host at all.
...
**Refresh.** The light post-install operation that reconciles this host's [Tenant Config](#tenant-config) data-dir items — the Project Map, the Tracker Credentials, and the project-docs — from the host's [Config Origin](#config-origin), no [Artifact](#artifact) present. It names that origin in what it reports, since a sync from the wrong dir and a sync from the right one are otherwise the same run to the operator reading it. Where a deploy's [Install](#install) places a whole Artifact, Refresh carries only the operator's own config edits across between deploys; a full framework reinstall — [Update](#update) — is the heavy path, this the light one. It reprimes the Foundations only when the inputs [Priming](#priming) reads have changed — the project-docs, or a Project Map that gained or dropped a Component — never for a rotated credential, so a config-only Refresh is a cheap sync, a `--dry-run` says in advance whether it would reprime, and a waivable confirmation guards the reprime when it would. Composed into the operator's [`vaudeville`](#vaudeville) CLI as `vaudeville refresh`: keeping a host in step with its config is the operator's call, and the machine [Installer](#installer) that places an Artifact never runs it. It touches no settings — the [Hook Wiring](#hook-wiring) and [Trust Declarations](#trust-declarations) are rebuilt as one unit that needs the Artifact, so they belong to a full install, not to Refresh.

## Update

**Update.** The heavy post-install operation that moves this host to a newer framework version: it downloads the newest published [Artifact](#artifact) — the same versioned download a first install places — and activates its carried [Installer](#installer) against the [Host](#destination), exactly as a first install does, so an upgrade and a first install cannot drift. Where [Refresh](#refresh) carries only the operator's own config edits with no Artifact present, Update fetches a whole new Artifact and reinstalls the framework — the doctrine, skills, hooks, and the `vv`/`vaudeville` tools themselves. It reaches GitHub for the Artifact through `gh` under the tenant's own authentication, never the integrator's [Published Home](#published-home) credential, which the Install half never holds. Composed into the operator's [`vaudeville`](#vaudeville) CLI as `vaudeville update`: moving a host to a newer framework is the operator's call, and its help says the upgrade is usually best left to an agent — a download or a config-schema change can surprise, and blowing up loud for an agent to judge beats a thicket of special-case code. Update is also the reserved home for [Tenant Config](#tenant-config) schema migrations: when a framework version changes the `vaudeville.toml` schema, the reconciliation will run here, between fetching the new Artifact and activating it, so it can abort before the new tools are placed — named now, not yet built, and bound by the rule that this command never silently rewrites the operator's own version-controlled config.
