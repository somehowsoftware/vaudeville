...
def _kill_process_group(process: subprocess.Popen[str]) -> None:
    ...
    # Name the fallback exceptions: `ruff format` miscompiles an inline `except (A, B):` into the
    # Python-2-only `except A, B:`, a SyntaxError under Python 3.
    killpg_errors = (ProcessLookupError, PermissionError)
    try:
        os.killpg(os.getpgid(process.pid), signal.SIGKILL)
    except killpg_errors:
        process.kill()
