...
def locate_youtrack(
    ...
    table = read_youtrack_credentials(credentials_path)
...
def read_youtrack_credentials(credentials_path: Path) -> dict[str, str]:
    # The shared reader for the Tracker Credentials, so a malformed one fails the same legible way
    # wherever it is read. An absent file is nothing to read here (a caller that requires it says
    # so); a present but unreadable or malformed one raises.
