...
def contribution_carries_a_wheel(contribution: Contribution) -> bool:
    ...
    # Command-Surface Contributors (those that offer a `vv` app) and the libraries they share. The
    # integrator offers no `vv` Surface — it is build-time tooling — yet may declare an
    # operator-only app whose code the carried installer ships, not the integrator. So the
    # wheel-carrying signal is a declared `binary`, a tenant `vv` Surface, not a CLI Declaration.
    declares_command_surface = (
        contribution.cli_declaration is not None and contribution.cli_declaration.binary is not None
    )
    return contribution.distribution is not None and (
        declares_command_surface or not contribution.console_scripts
    )
