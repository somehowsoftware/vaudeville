...
def snapshot(
    *,
    ...
    pr_view: JsonObject,
    ci_runs: Sequence[JsonObject] | None,
) -> Snapshot:
...
def _ci(ci_runs: Sequence[JsonObject] | None) -> Ci:
    # The caller encodes two sensed facts in this one channel: None when the repo has no ci.yml
    # workflow, an empty list when a workflow exists but no run is in yet for this head. It fetches
    # only the latest run, so the first is the one.
    if ci_runs is None:
        return Ci(status=CiStatus.ABSENT, run_handle=None)
    if not ci_runs:
        return Ci(status=CiStatus.AWAITING, run_handle=None)
    run = ci_runs[0]
