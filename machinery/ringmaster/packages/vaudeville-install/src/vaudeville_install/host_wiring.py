...
tenant's YouTrack — the tracker every Premise lives in — from ``credentials.toml``. On a fresh Host
either can be missing, and each surfaces only at the first spawn. This check turns those into a loud
Apply failure, exactly as the command-surface and Foundation probes do for Ringmaster's own surface
and the Foundations.
...
class HostWiringError(RuntimeError):
    ...
    def __str__(self) -> str:
        ...
        return (
            "Apply landed the Host Scaffold, but the host is not wired for `vv spawn`/`vv fork`:\n"
            f"{lines}\n"
