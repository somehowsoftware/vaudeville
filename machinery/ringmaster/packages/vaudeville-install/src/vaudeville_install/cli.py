"""The ``vaudeville-install`` entry point and composition root: the only module that supplies the
impure ``run`` and resolves the host environment, so no operation below reaches ``os.environ`` or
spawns for itself. Activated from inside a built Artifact on a host that has only ``uv``.
"""
