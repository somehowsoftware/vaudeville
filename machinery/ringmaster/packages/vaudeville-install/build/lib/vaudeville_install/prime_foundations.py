...
host-targeted environment, so priming cannot run against the rehearse shell's staged scratch.
"""
...
def prime_foundations(run_vv: RunVv) -> None:
    # `vv prime` with no prefix lets BOB prime every Component in its projects.toml. Pure over the
...
class FoundationsNotPrimed(RuntimeError):
    ...
    def __str__(self) -> str:
        base = (
            ...
            "Resolve the cause and re-run `vv prime`, or re-run the host install."
        )
