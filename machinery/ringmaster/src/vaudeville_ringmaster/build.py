"""Build: integrate an Apply Plan into a destination-ignorant Artifact."""
...
def build_artifact(plan: ApplyPlan, root: Path, *, build_wheel: BuildWheel) -> Artifact:
    if root.exists():
        shutil.rmtree(root)
    root.mkdir(parents=True)
    artifact = Artifact(root=root)
    _collect_files_into(plan, artifact)
    _merge_matchers_into(plan, artifact)
    carry_integrated_cli_into(plan, artifact.carried_cli, build_wheel)
    return artifact


def _collect_files_into(plan: ApplyPlan, artifact: Artifact) -> None:
...
def _merge_matchers_into(plan: ApplyPlan, artifact: Artifact) -> None:
    matchers = [c.hook_matchers for c in plan.contributions if c.hook_matchers is not None]
