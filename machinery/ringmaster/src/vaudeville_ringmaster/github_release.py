"""The `gh` boundary for Publish: run a `gh` command and interpret its outcome."""
...
def create_release_with_gh(
    *,
    ...
    asset: Path,
    target: str,
    run_gh: Callable[[list[str]], GhOutcome],
) -> None:
    outcome = run_gh(
...
def release_creator(run_gh: Callable[[list[str]], GhOutcome]) -> CreateRelease:
...
def tags_in(repository: str, run_gh: Callable[[list[str]], GhOutcome]) -> list[str]:
