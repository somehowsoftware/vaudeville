...
`ringmaster apply` installs the Host Scaffold, but by the time it runs the question "does the integrated set work?" must already be settled; Apply is not where breakage gets caught. Stage settles it first: it composes a Release Candidate and installs it as a Staged Scaffold structurally identical to what Apply would install, against which both the Audit and the lifecycle smoke run.

Whether a clean install also left a host that can actually be deployed-from is a separate question, answered after a Host install by the Integrity check and its host-wiring sibling (the carried installer runs both, described below). The Integrity check covers the scaffold; the host-wiring check covers the host *environment* `vv spawn`/`vv fork` reach past it for. The bar there is wiring, not Contributor semantics: whether the YouTrack credential authenticates or a real `workmux add` succeeds are deeper, Contributor-owned checks.
