...
Before either cut, two thresholds. **Is there a claim here at all?** A description of how something behaves (*the spawn path leaks worktrees*) asserts nothing a peer can argue with until you say what you would do about it and would defend that. Do not file a bare observation as a Premise; rewording it to sound like an opinion does not make it one. Find the real position under it and author that; or, if you will not work it out now, capture it with `/tangent` rather than inventing a position you do not yet hold.

**And is there an Assignment here at all?** A small change whose content is already settled by work in flight is a piece of that work, not an Assignment of any kind: it ships as a coordinated PR on the driving Assignment, now, rather than as a hand-off that transcribes the settled details across a boundary — transcription is where they corrupt. With nothing in flight to ride, a settled-and-small change is done now and recorded, when the operator wants residue, as a backfill Command marked delivered: their verb, their call. Surface the off-ramp as a suggestion, and file only what should stand alone.
...
A third reading runs on the draft rather than the goal: dictated means — contract details, mechanisms, paths, steps — are evidence about settledness, sorted by where each came from. Transcribed from in-flight sibling work: the coordinated-PR threshold above, not a kind question. Invented here to make the goal feel closed: the ends are not settled, a Premise. Neither, the call standing without them: backseat driving to cut, not a kind signal. `/direction`'s gate carries the full read; it runs there before filing.
