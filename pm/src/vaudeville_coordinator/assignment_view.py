...
- The shape of an Assignment as `vv assignment-show` and downstream skills
  see it (``AssignmentView``, plus ``DepView`` for one Depend-link peer and
  ``CommentView`` for one comment on the thread).
...
class CommentView(BaseModel):
    """One comment on the Assignment's thread, projected for display."""
...
class AssignmentView(BaseModel):
    ...
    def from_assignment(cls, assignment: Assignment) -> Self:
        ...
        return cls(
            ...
            comments=tuple(
                CommentView(author=comment.author, text=comment.text, created=comment.created)
