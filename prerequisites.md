...
The terrain that requires it is depth: systems whose parts are individually intelligible but whose total coordination burden exceeds what one mind can hold in stable relation, where the dominant risk is not bad code but a wrong abstraction quietly becoming structural under competent-looking work. On that terrain the bottleneck is never generation. It is keeping the system's meaning intact while a great deal of locally plausible work is applied to it, and that is the one job this framework is built to do.
...
The machinery's unit of grip is the [Component](doctrine/vocabulary.md#component), realized as one git repository. Priming compiles a [Foundation](doctrine/vocabulary.md#foundation) per Component; an [Assignment](doctrine/vocabulary.md#assignment) resolves to one; a [Bob](doctrine/vocabulary.md#bob) spawns into its repository's worktree and does its substantive work there. All of that assumes the repository boundary is a semantic boundary: that each repository is a [Context](doctrine/vocabulary.md#context), a jurisdiction with a domain of its own, or at least a single coherent responsibility, the bar a [Resource](doctrine/vocabulary.md#resource) clears without holding a domain.
