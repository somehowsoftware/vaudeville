...
and Workflow, not yet signed off. Those canonical values are the schema's own
per-field defaults, projected here so there is one source of truth. YouTrack
carries a field's default in the project custom field's ``defaultValues``; this
module points each essential field's default at its canonical value, resolving
the value's id against that field's own bundle, and leaves a field already at
its canonical default alone. A project missing an essential field, or a field
whose bundle holds no value of the canonical name, is a violation the run aborts
on rather than skips.
...
writes. Provisioning drives this same pure core as its terminal round; this
module also runs standalone, re-setting the defaults on already-registered
projects.
"""
...
from vaudeville_core.tracker_schema import TRACKER_SCHEMA

ESSENTIAL_DEFAULTS: dict[str, str] = {field.name: field.default for field in TRACKER_SCHEMA.fields}
...
def project_fields_request(project_id: str) -> Request:
    return Request(
        ...
        params={"fields": "id,$type,field(name),bundle($type,values(id,name)),defaultValues(name)"},
    )
...
def current_default(field: dict[str, Any]) -> str | None:
    defaults = field.get("defaultValues") or []
    return str(defaults[0]["name"]) if defaults else None
...
def essential_default_requests(project_id: str, fields: list[dict[str, Any]]) -> list[Request]:
    ...
    for field_name, value_name in ESSENTIAL_DEFAULTS.items():
        ...
        if current_default(field) == value_name:
            continue
