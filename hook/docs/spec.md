...
A screen's friction never falls on its own remedy. A perception is resolved by an action the Bob takes — fix the work, shed the context, record why the screen is wrong — and that action is often several steps of its own. Friction laid across those steps bars the one path the perception exists to compel: a per-turn block fired through the multi-step checkpoint it is demanding is not high friction but a wall, and a deadlock only an outside hand can break. So a screen holds its friction off the remedy, sensing when the Bob is already carrying it out and standing down until it completes.
