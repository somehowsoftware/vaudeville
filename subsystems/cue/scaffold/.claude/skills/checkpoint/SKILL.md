...
So the skill is complete the moment you invoke the command. **Produce nothing after it**: not a summary, not a sign-off, not a "checkpoint launched," not one more tool call. There is no next step for you, and a turn still in flight when the drive's `/clear` is applied can break the handoff itself. The Carryover was your last word, and you have already delivered it. Run the command; stop.
