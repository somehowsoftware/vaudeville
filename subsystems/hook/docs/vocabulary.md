...
A Bob-authored, per-session postponement of the *soft* [rungs](#ladder-and-rung): a ceiling and a reason the Bob records, by which it dismisses the gentle and active rungs under a justification the screen reads each tick. A stay raises only the soft rungs; it cannot suppress the aggressive or emergency rungs, and it cannot move the [wall](#the-wall) — those are the backstop, and the backstop is physics. A stay is bound to the session that wrote it: a checkpoint opens a fresh session, so a stale stay self-invalidates rather than needing a remembered reset to clear it. Named *stay* (over *slack*, *grant*, *reprieve*) so that "the stay moved the wall" reads as the nonsense it is: a stay postpones a soft rung, it does not buy headroom that physics did not grant.
