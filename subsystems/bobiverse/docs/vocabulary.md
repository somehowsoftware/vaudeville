...
A recorded Foundation (`~/.vaudeville/doctrine/vocabulary.md#foundation`) is **durable** when a later [Spawn](#spawn) can fork from it on this host, regardless of which clone the Bob runs in; **stranded** when it is recorded but not present on the host, so a Spawn cannot fork from it: for example a [Prime](#prime) performed inside a smoke that never reaches the host. `vv foundations-verify` reports the stranded Foundations; Ringmaster's post-apply integrity check invokes it so a deploy that left a Foundation stranded fails loudly at apply time rather than at the next Spawn.
