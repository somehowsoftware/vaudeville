"""A session transcript parsed into messages, the value the Digest is derived from.

Claude Code records a session as JSONL — one message record per line. This module
turns those lines into a uniform list of `Message` values with typed content
blocks, so the operator-turns extraction downstream reads one representation
rather than walking raw JSON. Parsing takes already-read lines and returns values;
