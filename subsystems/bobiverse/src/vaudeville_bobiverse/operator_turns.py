...
from vaudeville_bobiverse.digest import OperatorTurn
from vaudeville_bobiverse.resume_brief import RESUME_BRIEF_OPENING
from vaudeville_bobiverse.transcript import Message, ToolResult, ToolUse

# A user-role record is the operator speaking only when the harness recorded that
# the operator typed or queued it. Every other source under the user role —
# "system" (task-notifications), "sdk" (a spawned session's seed Brief), the absent
# source (tool results, harness meta) — is machinery injected into the conversation,
# not a turn.
