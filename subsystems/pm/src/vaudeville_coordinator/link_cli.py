...
Adapter between Typer dispatch and "core"'s Depend-edge operations.
Each handler validates the Assignment-id shape on both sides of an edge
...
the composition root binds the real "core" op, tests bind a fake.
"""
...
def depend_add(source: str, target: str, *, add_depend: DependEdge) -> None:
    component_from_assignment_id(source)
    component_from_assignment_id(target)
